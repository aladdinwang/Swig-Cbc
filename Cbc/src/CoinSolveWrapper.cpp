...
std::string cbc_main2(int argc, const char *argv[]){
  ...
  OsiClpSolverInterface solver1;
  //load mps from string
  solver1.setLogLevel(0);
  solver1.getModelPtr()->setOptimizationDirection(-1.0);
  solver1.readMps(argv[1], true, false);
  CbcModel model(solver1);
  ...
  for (iColumn=0;iColumn<numberColumns;iColumn++) {
    ...
    if (fabs(value)>1.0e-7&&solver->isInteger(iColumn)){
      ss<<std::setw(6)<<iColumn<<" "<<std::setw(8)<<std::setiosflags(std::ios::left)<<solver1.getModelPtr()->columnName(iColumn)
        <<std::resetiosflags(std::ios::adjustfield)<<std::setw(14)<<" "<<value<<std::setw(14)<<" "<<0<<std::endl;      
    }else{
      ss<<std::setw(6)<<iColumn<<" "<<std::setw(8)<<std::setiosflags(std::ios::left)<<solver1.getModelPtr()->columnName(iColumn)
        <<std::resetiosflags(std::ios::adjustfield)<<std::setw(14)<<" "<<0<<std::setw(14)<<" "<<0<<std::endl;  
    }
  }  
  return ss.str();
}
