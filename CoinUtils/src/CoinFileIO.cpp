...
class CoinStreamFileInput: public CoinFileInput
{
public:
  CoinStreamFileInput(const std::string &fileName):
    CoinFileInput(""), s_(fileName){

    };
  virtual ~CoinStreamFileInput(){}

  virtual int read(void *buffer, int size)
  {
     return s_.readsome(static_cast<char*>(buffer), size);
  }


  virtual char *gets(char *buffer, int size)
  {
      s_.getline(buffer, size);
      return buffer;
  }
...
CoinFileInput *CoinFileInput::create (const std::string &fileName)
{
  ...
  // hack CoinFileInput 
  return new CoinStreamFileInput(fileName);
